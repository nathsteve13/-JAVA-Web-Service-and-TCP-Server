...
public class Parking_reservations extends MyModel {
    ...
    public int getParkings_id() {
        return parkings_id;
    }

    public void setParkings_id(int parkings_id) {
        this.parkings_id = parkings_id;
    }
    ...
    public void insertData() {
        try {
            if (!MyModel.conn.isClosed()) {
                ...
                sql.setInt(1, this.accounts_id);
                sql.setInt(2, this.parkings_id);
    ...
    public void updateData() {
        try {
            if (!MyModel.conn.isClosed()) {
                PreparedStatement sql = MyModel.conn.prepareStatement(
                        "UPDATE parking_reservations SET accounts_id = ?, parkings_id = ?, quantity = ?, amount = ?, status = ?, claim_date = ?, claimed_date = ?, updated_at = ?, created_at = ? WHERE accounts_id = ? AND parkings_id = ?");
                sql.setInt(1, this.accounts_id);
                sql.setInt(2, this.parkings_id);
                sql.setInt(3, this.quantity);
                sql.setDouble(4, this.amount);
                ...
                sql.setDate(8, java.sql.Date.valueOf(this.updated_at));
                sql.setDate(9, java.sql.Date.valueOf(this.created_at));
                sql.setInt(10, this.accounts_id);
                sql.setInt(11, this.parkings_id);
                sql.executeUpdate();
                sql.close();
            }
        } catch (Exception ex) {
    ...
    public ArrayList<String> viewListData() {
        ...
        try {
            ...
            while (result.next()) {
                ...
                tempReservation.setAccounts_id(result.getInt("accounts_id"));
                tempReservation.setParkings_id(result.getInt("parkings_id"));
                ...
                collections.add(tempReservation.getAccounts_id() + "-" + tempReservation.getParkings_id() + "-" + tempReservation.getQuantity() + "-" + tempReservation.getAmount() + "-" + tempReservation.getStatus() + "-" + tempReservation.getClaim_date() + "-" + tempReservation.getClaimed_date() + "-" + tempReservation.getUpdated_at() + "-" + tempReservation.getCreated_at());
            }
        } catch (Exception ex) {
            System.out.println(ex.getMessage());
        } finally {
