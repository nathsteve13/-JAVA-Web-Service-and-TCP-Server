...
public class Event_reservations extends MyModel{
    ...
    public void insertData() {
        try {
            if (!MyModel.conn.isClosed()) {
                ...
                sql.setInt(1, this.account_id);
                sql.setInt(2, this.event_id);
    ...
    public void updateData() {
        try {
            if (!MyModel.conn.isClosed()) {
                PreparedStatement sql = MyModel.conn.prepareStatement(
                        "UPDATE event_reservations SET account_id = ?, event_id = ?, quantity = ?, amount = ?, status = ?, claim_date = ?, claimed_date = ?, updated_at = ?, created_at = ? WHERE account_id = ? AND event_id = ?");
                sql.setInt(1, this.account_id);
                sql.setInt(2, this.event_id);
                sql.setInt(3, this.quantity);
                sql.setDouble(4, this.amount);
                ...
                sql.setDate(8, java.sql.Date.valueOf(this.updated_at));
                sql.setDate(9, java.sql.Date.valueOf(this.created_at));
                sql.setInt(10, this.account_id);
                sql.setInt(11, this.event_id);
                sql.executeUpdate();
                sql.close();
            }
        } catch (Exception ex) {
